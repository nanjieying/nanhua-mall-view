  ...
  <div>
    ...
    <div class="goods-list-container">
      <Alert show-icon class="tips-box">
        <Icon type="ios-lightbulb-outline" slot="icon"></Icon>小提示
        <template slot="desc">填写并核对订单信息</template>
      </Alert>
      <!-- 收货地址区域 -->
      <div class="address-container">
        ...
        <div class="address-box">
          <div class="address-check">
            ...
            <button style="border: none;width: 120px;background-color: #f90013;color: #ffffff;line-height:36px;margin-left:15px;" @click="skipAddress">新增收货地址</button>
      ...
      <div class="address-container">
        ...
        <div class="address-box">
          ...
          <Row
            v-for="(item,index) in selectedCartInfo.selectedCartItems"
            :key="index"
            class="pay-order-row pay-order-col"
          >
            <Col span="4">
            ...
            <Col span="4">
              <div>{{item.specialPrice.toFixed(2)}}</div>
            </Col>
            <Col span="4">
      ...
      <div class="address-container">
        <h3>
          使用优惠/礼品卡/抵用
          <Icon
            :type="discountShowValue===false ? 'ios-arrow-down' : 'ios-arrow-up'"
            @click="changeShowDiscount()"
          ></Icon>
        </h3>
                    ...
                    <ul>
                      <li v-for="(item, index) in item1.coupons" :key="index">
                        <div class="coupon-item c-nopointer coupon-item-new">
                          <div
                            class="c-detail"
                            :class="{'c-detail item-selected':item.checked}"
                            @click="selectCoupon($event,item1,item,index1,index)"
                          >
      ...
      <Row class-name="topDist">
        <Col span="3" offset="20" class="order-summary-item">
          <span>
            <em class="ftx-01">{{num}}</em> 件商品，总商品金额：
          </span>
        </Col>
        <Col span="1">
      ...
      <Row>
        <Col span="3" offset="20" class="order-summary-item">商品优惠 :</Col>
        <Col span="1">
    ...
    methods: {
      ...
      selectCoupon(e, item1, item, index1, index) {
        ...
        if (!item.checked) {
          ...
          // 更新优惠价 -
          this.couponPrice -= item.amount
          this.selectedCouponCidList.splice(
            this.selectedCouponCidList.findIndex(v => v === cid),
            1
          )
          this.selectedCouponIdList.splice(
            this.selectedCouponIdList.findIndex(v => v === item.id),
            1
          )
          item.checked = false
          this.couponList[index1].coupons.splice(index, 1, item)
...
<style scoped>
  ...
  .trade-foot-detail-com .presale-price-info {
    margin-right: 10px;
    line-height: 25px;
  }
  ...
  .coupon-scroll .coupon-item .c-detail .c-msg .c-over {
    color: #f5f5f5;
    font-size: 12px;
    display: inline;
    position: relative;
