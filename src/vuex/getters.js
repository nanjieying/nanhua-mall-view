...
export const merchantItem = state => {
  ...
  for (let i = 0; i < all.length; i++) {
    const temp = {
      goodsId: all[i].id,
      goodsName: all[i].productName.substring(0, 30) + '...',
      relCategory3Id: all[i].relCategory3Id,
      merchantName: all[i].tenantName,
      merchantId: all[i].relTenantId,
      img: all[i].defaultPic,
      price: all[i].price,
      sale: all[i].salesNum
    };
    asItem.push(temp);
...
export const getGoodsDetailBase = state => {
  const goodsImgs = state.goodsDetail.albumPics ? state.goodsDetail.albumPics.split(',') : [];
  const data = {
    goodsImgs: goodsImgs,
...
export const getGoodsDetail = state => {
  const data = {
    ...
    detailDesc: state.goodsDetail.detailDesc,
    // 暂时以多图逗号分割处理
    detailHtml: state.goodsDetail.detailHtml.split(','),
    // 模拟数据
