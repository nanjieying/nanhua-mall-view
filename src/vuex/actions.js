...
export const loadRecommend = ({ commit }, data) => {
  return new Promise((resolve, reject) => {
    recommendApi.recommendLookProduct(data.relProductId).then(res => {
      if (res.code === 200) {
...
export const isExist = ({ commit }, data) => {
  return new Promise((resolve, reject) => {
    baseApi.isExist(data).then(res => {
      console.log('判断手机是否存在')
      console.log(res)
      const data = res.data
      const result = { isExist: data }
      if (!data) {
...
export const loadPopRecommends = ({ commit }) => {
  return new Promise((resolve, reject) => {
    ...
    commit('SET_POPRECOMMENDS_INFO', recommends)
  })
}
...
export const addShoppingCart = ({ commit }, data) => {
  ...
  return new Promise((resolve, reject) => {
    cartApi.addShoppingCart(item).then(res => {
      if (res.code === 200) {
...
export const getGoodsByName = ({ commit }, data) => {
  ...
  return new Promise((resolve, reject) => {
    baseApi.goodsList(data).then(res => {
      ...
      commit("SET_TOTAL_GOODS_NUMS", res.data.totalElements - 1)
